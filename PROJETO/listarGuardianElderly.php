<?php
	...
	$resultIdElderly = $registroElderly['ID'];//id do idoso na tabela people
	...
	$registroGuardian = mysqli_fetch_array($idGuardian);//pegando o id do guardião na tabela guardian_has_elderly

	if(!$registroGuardian['ID']){//verificando se o id do guardião está vazio
		$resultIdGuardian = " ";
		...
		$registroGuardianPeople = mysqli_fetch_array($idGuardianPeople); //pegando o people_id do guardião
		

		if(!$registroGuardianPeople['ID']){//verificando se está vazio
			$resultIdGuardianPeople = " ";
		}else{
			$resultIdGuardianPeople = $registroGuardianPeople['ID'];
			$sql = "SELECT people.id AS 'ID_PEOPLE', 
			people.name AS 'NOME', 
			guardian_has_elderly.guardian_id AS 'ID' 
			FROM people 
			INNER  JOIN guardian 
			ON guardian.people_id=people.id  
			INNER JOIN guardian_has_elderly 
			ON guardian_has_elderly.elderly_id = '$resultIdElderly' 
			AND guardian.people_id='$resultIdGuardianPeople';";
...
<body>
	<div class="container mt-3">
			...
			<tbody>
				<?php
					while ($registro = mysqli_fetch_array($resultado)) {
						...
						if(!$registro){
							"<tr>
								<td colspan='6'> Sem guardiões cadastrados.</td>
							</tr>";
						}else{
							echo "<tr>
									<td>" . $registro['ID']   . "</td>
									<td>" . $registro['NOME'] . "</td>
									<td><a href=updateGuardianElderly.php?idGuardianPeople=".$registro['ID_PEOPLE']."&idIdosoPeople=".$resultIdElderly.">
		...
		<a href='guardianRegistration.php?idIdosoPeople=$resultIdElderly' style='color: #000000;'>
